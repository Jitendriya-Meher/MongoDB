...
const getDocument = async ()=>{
    ...
    try{
        ...
        // find method
        const result1 = await Playlist.find();

        console.log("result 1 : ",result1);

        const result2 = await Playlist.find({ctype:"Back End"});

        console.log("result 2 : ",result2);

        const result3 = await Playlist.find({ctype:"Back End"}).select({name:1});

        console.log("result 3 : ",result3);

        const result4 = await Playlist.find({ctype:"Back End"}).select({name:1}).limit(1);
